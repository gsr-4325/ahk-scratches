    ...
    while True:
        if worker.proc.poll() is not None:
            ...
            raise HarnessFailure(
                f"{worker.worker_id} exited before {label}. exit_code={worker.proc.returncode} stderr={stderr_text!r}"
            )
        ...
        if remaining <= 0:
            stderr_text = ''.join(worker.stderr_chunks).strip()
            raise HarnessFailure(
                f"Timeout waiting for {label} from {worker.worker_id} after {timeout_seconds:.1f}s; child_exit={worker.proc.poll()} stderr={stderr_text!r}"
            )
        ...
        if kind == 'eof':
            stderr_text = ''.join(worker.stderr_chunks).strip()
            raise HarnessFailure(f'stdout closed for {worker.worker_id} while waiting for {label}; stderr={stderr_text!r}')
...
def start_worker(worker_id: str, ahk_exe: str, child_path: Path) -> WorkerProc:
    ...
    stdout_thread.start()
    stderr_thread.start()
    return WorkerProc(worker_id, proc, stdout_queue, stderr_chunks, stdout_thread, stderr_thread)
...
def main() -> int:
    ...
    artifact_dir = Path(os.environ.get('ARTIFACT_DIR', repo_root / 'artifacts-ipc-three-workers-v1'))
    ...
    try:
        for worker_id in worker_ids:
            worker = start_worker(worker_id, ahk_exe, child_path)
        ...
        line = send_and_expect(workers[0], 'hello one', args.timeout_seconds, expected='echo=hello one')
        transcript.append('worker-001 > hello one')
        transcript.append(f'worker-001 < {line}')

        line = send_and_expect(workers[1], 'hello two', args.timeout_seconds, expected='echo=hello two')
        transcript.append('worker-002 > hello two')
        transcript.append(f'worker-002 < {line}')

        line = send_and_expect(workers[2], 'hello three', args.timeout_seconds, expected='echo=hello three')
        transcript.append('worker-003 > hello three')
        transcript.append(f'worker-003 < {line}')
...
if __name__ == '__main__':
    try:
        ...
        print(f'[three-workers-v1] {type(exc).__name__}: {exc}', file=sys.stderr)
        raise
