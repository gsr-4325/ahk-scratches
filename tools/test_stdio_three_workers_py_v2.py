...
def start_worker(worker_id: str, ahk_exe: str, child_path: Path) -> WorkerProc:
    proc = subprocess.Popen(
        [ahk_exe, '/ErrorStdOut', str(child_path), worker_id],
...
def main() -> int:
    ...
    child_path = repo_root / 'scripts' / 'tests' / 'stdio' / 'child_echo_multiworker_v1.ahk'
    artifact_dir = Path(os.environ.get('ARTIFACT_DIR', repo_root / 'artifacts-ipc-three-workers-v2'))
...
if __name__ == '__main__':
    try:
        ...
        print(f'[three-workers-v2] {type(exc).__name__}: {exc}', file=sys.stderr)
        raise
